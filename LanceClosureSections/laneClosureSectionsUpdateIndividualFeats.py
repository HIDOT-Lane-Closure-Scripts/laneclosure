...
lnclfeats = lnclsectqry.features
scols = lnclsectsdf.columns
for feat in lnclfeats:
    feat.attributes['beginDate'] = (datetime.fromtimestamp(feat.attributes['beginDate']/1e3) - timedelta(hours=10)) 
    feat.attributes['enDate'] = (datetime.fromtimestamp(feat.attributes['enDate']/1e3) - timedelta(hours=10)) 
    feat.attributes['begDyWk']= (datetime.strftime(feat.attributes['beginDate'],"%A")) # srow.begDyWk = datetime.strftime(srow.beginDate,"%A") 
    feat.attributes['enDyWk'] = (datetime.strftime(feat.attributes['enDate'],"%A")) # srow.enDyWk =  datetime.strftime(srow.enDate,"%A")
    feat.attributes['todayis'] = (datetime.today()) #srow.todayis = datetime.today()
    feat.attributes['Friday0'] = (fridaywk(feat.attributes['todayis'],0)) #srow.Friday0 = fridaywk(srow.todayis,0) 
    feat.attributes['Friday1'] = (fridaywk(feat.attributes['todayis'],1)) #srow.Friday1 = fridaywk(srow.todayis,1)
    feat.attributes['Friday2'] = (fridaywk(feat.attributes['todayis'],2)) #srow.Friday2 = fridaywk(srow.todayis,2)
    insecgeo = feat.attributes['SHAPE']
    ...
    try:
        ...
        logger.error('Layer {} Update with day of week and friday data failed.  Error Message  :{}'.format(
                wmlnclyrptsnm,e ))
        intsfromtxt = "Beginning at Mile Post {}".format(feat.attributes['BMP'])
        intstotxt = "Ending at Mile Post {}".format(feat.attributes['EMP'])
    
    feat.attributes['IntersFrom'] = intsfromtxt  # arcgis.geocoding.reverse_geocode(insecgeo[0],distance=200,out_sr=4326,return_intersection=True,for_Storage=True)
    feat.attributes['IntersTo'] = intstotxt #arcgis.geocoding.reverse_geocode(insecgeo[len(insecgeo)-1],distance=200,out_sr=4326,return_intersection=True,for_Storage=True)

    lnclupdatefeatset = [feat]
    fset = arcgis.features.FeatureSet(lnclupdatefeatset)
    try:
        resultupd1 = wmlnclyrsects.edit_features(updates=fset) #calculate(where=begwkqry,calc_expression={"field" : fld1 , "value" : val1})
