...
logfilenm = 'socratransdetail.log'
...
sectsdfwgs84['CreationDate'] = sectsdf['CreationDate'].apply(lambda xt : xt - timedelta(hours=10))  # + timedelta(hours=-10) # [datetime.fromtimestamp(round(datetime.timestamp(xt)/1e3,0)) for xt in sectsdfwgs84['CreationDate']]  #[(int(xt/1e3)) for xt in sectsdf['CreationDate']] # [ datetime.fromtimestamp(xt) for xt in sectsdf['CreationDate']]
sectsdfwgs84['beginDate'] = sectsdf['beginDate'].apply(lambda xt : xt - timedelta(hours=10)) #[ xt - timedelta(hours=10) for xt in sectsdf['beginDate']]  # + timedelta(hours=-10) # [datetime.fromtimestamp(round(datetime.timestamp(xt)/1e3,0)) for xt in sectsdfwgs84['beginDate']] # [ datetime.fromtimestamp(xt) for xt in sectsdf['CreationDate']]
sectsdfwgs84['enDate']= sectsdf['enDate'].apply(lambda xt : xt - timedelta(hours=10)) #[ xt - timedelta(hours=10) for xt in sectsdf['enDate']]  # + timedelta(hours=-10)# [datetime.fromtimestamp(round(datetime.timestamp(xt)/1e3,0)) for xt in sectsdfwgs84['enDate']] # [ datetime.fromtimestamp(xt) for xt in sectsdf['CreationDate']]

#sectsdfwgs84['begWkDy'] = [xt if np.isnan(xt) else date.strftime((xt- timedelta(hours=0),"%w")) for xt in sectsdfwgs84['beginDate']]  #[(int(xt/1e3)) for xt in sectsdf['CreationDate']] # [ datetime.fromtimestamp(xt) for xt in sectsdf['CreationDate']]
...
sectsdfwgs84['begWkDy'] = sectsdfwgs84['beginDate'].apply(lambda xt : date.strftime(xt- timedelta(hours=0),"%w"))   #[(int(xt/1e3)) for xt in sectsdf['CreationDate']] # [ datetime.fromtimestamp(xt) for xt in sectsdf['CreationDate']]
...
sectsdfwgs84['endWkDy'] = sectsdfwgs84['enDate'].apply (lambda xt : date.strftime(xt- timedelta(hours=0),"%w"))  #[(int(xt/1e3)) for xt in sectsdf['CreationDate']] # [ datetime.fromtimestamp(xt) for xt in sectsdf['CreationDate']]
...
sectsdfwgs84['LaneCLineS']=  sectsdfwgs84['SHAPE'].apply(lambda x : soclinegometry(x) )
...
sectsdfwgs84['SHAPE']=  sectsdfwgs84['SHAPE'].apply(lambda x : str(x)) 
...
#lncldetailfile = r"\\HWYPB\NETSHARE\HWYA\HWY-AP\LaneClosure\{}detail.csv".format(wmlnclyrsectsnm)
lncldetailfilenm = r"{}detail.csv".format(wmlnclyrsectsnm)
lncldetailfile = os.path.join(rptpath,lncldetailfilenm)

#try:
#    lnclcsv = sectsdfwgs84.to_csv(lncldetailfile,encoding='utf-8')
#except Exception as e:
...
try:
    ...
    (ok1, vwdetsect) = laneclosocrata.views.lookup(lncldetailid)
    assert(ok1)# Create a revision on this view. You can also choose create_update_revision() to append data.
    (ok2, revdetsect) = vwdetsect.revisions.create_replace_revision()
    assert(ok2)# Create an upload to this revision based on our file
    mergedata = 'Lane_Closure_detail_Data_Replace'
    (ok3, upldetsect) = revdetsect.create_upload(mergedata)
    assert(ok3)# Actually put data into the upload with a source.
    (ok4, srcdetsect) = upldetsect.df(sectsdfwgs84)
    assert(ok4)# Identify the input and output schemas.
    detsect_schema = srcdetsect.get_latest_input_schema().get_latest_output_schema()# Apply the revision based on the output schema.
    (ok5, jobrevdf) = revdetsect.apply(output_schema=detsect_schema)
    assert(ok5)
    (ok6, jobrevdf) = jobrevdf.wait_for_finish(progress = lambda jobrevdf: print('Job progress:', jobrevdf.attributes['status']))

    # So maybe we just want to wait here, printing the progress, until the job is done
    jobrevdf.wait_for_finish(progress = lambda jobrevdf: print(jobrevdf.attributes['log']))
    tend=datetime.today().strftime("%A, %B %d, %Y at %H:%M:%S %p")
    if jobrevdf.attributes['status'] == 'successful':
        logger.info (" {} End of lane closure Socrata  id {} processed {} features , Update Result {} .\n Grouped Table id {} processed {} columns. {} ".format (jobrevdf.attributes['status'],lncldetailid,len(sectfeat),ok6,lncldetailid,len(sectsdfwgs84,tend)))
        print (" {} End of lane closure Socrata  id {} processed {} features , Update Result {} .\n Grouped Table id {} processed {} columns. {} ".format (jobrevdf.attributes['status'],lncldetailid,len(sectfeat),ok6,lncldetailid,len(sectsdfwgs84),tend))
    else:    
        logger.info (" {} end of Lane closure Socrata id {} with {} features , Update Result {} .\n Grouped Table id {} processed {} columns. Update results {} at {}".format (jobrevdf.attributes['status'],lncldetailid,len(sectfeat),view1,lncldetailid,len(sectsdfwgs84),ok6,tend))
