...
fsectestWebMapTitle = 'Lane_Closure_Test_WFL' # arcpy.GetParameter(0) #  'e9a9bcb9fad34f8280321e946e207378'
itypeFStest="Feature Service" # "Feature Layer" # "Service Definition"
wmlnclyrptestnm = 'Lane_Closure_Begin_and_End_Points_Test'
wmlnclyrsectestnm = 'Lane_Closure_Sections_Test'
...
fslnclsrctest = webexsearch(qgis, fsectestWebMapTitle, userName, itypeFStest,numfs,ekOrg)
...
print (" Feature URL: {} ; Title : {} ; Id : {} ".format(fslnclsrctest.url,fslnclsrctest.title,fslnclsrctest.id))
lnclyrsrctest = fslnclsrctest.layers

wmsectlyrpts = lyrsearch(lnclyrsrctest, wmlnclyrptestnm)
wmlnclyrsects = lyrsearch(lnclyrsrctest, wmlnclyrsectestnm)


# search for lane closure sections 
print("Searching for {} from {} item for user {} and Service Title {} on AGOL...".format(itypeFStest,portal_url,userName,fsectestWebMapTitle))
...
fld0 = "begDyWk" # "Creator" #
fld1 = "beginDate"
val0 = "!{}!.strftime('%A')".format(fld1) 
qryStr0 = "1=1" #{} like '{}' ".format(fld0,val0)
lnclqry = wmlnclyrsects.query(where=qryStr0) # lnclhdrlyr.query(where=qryStr0)
...
#        email1 = lfs.get_value('l1email') # ,'albert.chung@hawaii.gov')
    expn0 = {"field" : fld0 , "value" : val0}
    # {"field" : "ApproverL2" , "value" : "dot_mmedeiros"}) 
    resultupd1 = wmlnclyrsects.calculate(where=qryStr0,calc_expression={"field" : fld0 , "value" : val0})
    print (" Update result {} for  {} ".format (resultupd1,expn0))
